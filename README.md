...
> - 密码以**明文**写入本地 `settings.yaml`，注意凭证安全。
>
...
保存后 Host 自动重连；也可以直接在对话里问模型 `abap_mcp_status` 查看连接状态。
...
- 密码以**明文**写入本地 `settings.yaml`（由 DSH 的 settings 服务持久化）。如需更高安全级别，请自行扩展为 `credentials` 服务存储。
