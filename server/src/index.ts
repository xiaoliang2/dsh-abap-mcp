...
export class AbapAdtServer extends Server {
  ...
  private objectRegistrationHandlers: ObjectRegistrationHandlers;
    ...
    console.error(`[dsh-abap-mcp] Permission mode: ${this.permissions.describe()}`);
    if (this.permissions.isReadOnly()) {
      console.error(
        '[dsh-abap-mcp] Read-only mode. Recommended for dev/test systems with a least-privilege account; enable optional categories via MCP_ABAP_PERMISSIONS only when you actually need them.'
      );
    }
    // Initialize handlers
  ...
  private setupToolHandlers() {
    this.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      for (const t of allTools) {
        if (!this.permissions.isKnown(t.name)) {
          console.warn(
            `[dsh-abap-mcp] Tool '${t.name}' is not modeled in src/permissions.ts and is DISABLED by default — add it there if it should be gated or allowed.`
          );
        }
      }
      return { tools: allTools.filter((t) => this.permissions.isEnabled(t.name)) };
    });

    this.setRequestHandler(CallToolRequestSchema, async (request) => {
      try {
        if (!this.permissions.isEnabled(request.params.name)) {
          throw new McpError(
            ErrorCode.MethodNotFound,
            `Tool '${request.params.name}' is disabled: the server runs in ${this.permissions.isReadOnly() ? 'read-only' : 'restricted'} mode. Enable the required permission category via MCP_ABAP_PERMISSIONS if you need it.`
          );
        }
        let result: any;
