...
const ConfigSchema = z
  .object({
    ...
    // 写前预览（瞬态，非持久配置）：Host 在写工具执行前算好的 旧→新 diff 预览，
    // 通过设置命名空间下发给 Client 工具卡（tool.call.toolview）渲染「序号+点击展开」。
    // 不参与重连判定、不进服务器环境；用后由 Host 定时清空，避免长期留存源码。
    writePreview: z.any().default(null),
...
// ---------------- 写前预览：真实 旧→新 diff ----------------
// 目标：让用户在确认写入前能看到「到底会改成什么」，而不是只给一段截断的源码。
// 分两层：审批卡 reason 内联紧凑 diff（原生卡必显示）；完整 hunk 经设置命名空间
// 下发给 Client 工具卡，渲染「序号 + 点击展开」，改动再多也不省略。
const MAX_INLINE_REASON_HUNKS = 5; // reason 内联展示的前 N 处改动
const MAX_DIFF_CELLS = 200_000; // LCS 网格上限，超出退化为单块全量差异
const PREVIEW_TTL_MS = 120_000; // 预览在设置命名空间中的存活时间（用后自动清空）

function splitDiffLines(s) {
  return String(s ?? "").replace(/\r\n/g, "\n").replace(/\r/g, "\n").split("\n");
}

// 对中间段做有界 LCS diff，返回 ops：{t:'same'|'old'|'new', text}
function diffMiddle(a, b) {
  const n = a.length;
  const m = b.length;
  if (n === 0) return b.map((t) => ({ t: "new", text: t }));
  if (m === 0) return a.map((t) => ({ t: "old", text: t }));
  if (n * m > MAX_DIFF_CELLS) {
    return [
      ...a.map((t) => ({ t: "old", text: t })),
      ...b.map((t) => ({ t: "new", text: t })),
    ];
  }
  const w = m + 1;
  ...
  for (let i = n - 1; i >= 0; i--) {
    for (let j = m - 1; j >= 0; j--) {
      const idx = i * w + j;
      if (a[i] === b[j]) dp[idx] = dp[(i + 1) * w + j + 1] + 1;
      else dp[idx] = Math.max(dp[(i + 1) * w + j], dp[i * w + j + 1]);
    }
  }
  const ops = [];
  let i = 0;
  ...
  while (i < n && j < m) {
    const idx = i * w + j;
    if (a[i] === b[j]) {
      ops.push({ t: "same", text: a[i] });
      i++;
      j++;
    } else if (dp[(i + 1) * w + j] >= dp[i * w + j + 1]) {
      ops.push({ t: "old", text: a[i] });
      i++;
    } else {
      ops.push({ t: "new", text: b[j] });
      j++;
    }
  }
  while (i < n) {
...
// 计算 旧→新 的行级 diff，返回 { hunks, add, del }。
// hunk = { index, oldStart, newStart, lines:[{t,text}] }（1-based，含 1 行上下文）。
function computeLineDiff(oldText, newText) {
  ...
  const ops = diffMiddle(a.slice(pre, a.length - suf), b.slice(pre, b.length - suf));
  // 组装 hunk（含前后各 1 行上下文，便于定位）
  const hunks = [];
  let add = 0;
  let del = 0;
  let idx = 0;
  while (idx < ops.length) {
    ...
    // 收集一处连续改动块
    const start = Math.max(0, idx - 1); // 前 1 行上下文
    let end = idx;
    while (end < ops.length && ops[end].t !== "same") end++;
    end = Math.min(ops.length, end + 1); // 后 1 行上下文
    const lines = ops.slice(start, end).map((op) => ({ t: op.t, text: op.text }));
    // 计算该 hunk 在旧/新版本中的起始行号：统计它之前 旧/新 行数量
    let oldBefore = 0;
    let newBefore = 0;
    for (let k = 0; k < start; k++) {
      if (ops[k].t === "old") oldBefore++;
      else if (ops[k].t === "new") newBefore++;
      else {
        oldBefore++;
        newBefore++;
      }
    ...
    // 加上前缀公共行
    const oldStart = pre + oldBefore + 1;
    const newStart = pre + newBefore + 1;
    const hAdd = lines.filter((l) => l.t === "new").length;
    const hDel = lines.filter((l) => l.t === "old").length;
    add += hAdd;
    del += hDel;
    hunks.push({ index: hunks.length + 1, oldStart, newStart, lines });
    idx = end;
  }
  return { hunks, add, del, oldLines: a.length, newLines: b.length };
}
...
// 构造写前预览。返回 null 表示该工具没有可预览的源码内容（沿用摘要式 reason）。
// gen：当前连接的 MCP 客户端；rawToolName：服务器侧工具名；args：调用参数。
async function buildWritePreview(gen, rawToolName, args) {
  const a = args && typeof args === "object" ? args : {};
  const label =
    a.objectName || a.name || a.adtcoreName || a.adtcore_name || a.objectSourceUrl || rawToolName;
  const newSource =
    typeof a.source === "string" ? a.source : typeof a.content === "string" ? a.content : typeof a.code === "string" ? a.code : "";
  if (rawToolName === "setObjectSource") {
    const url = a.objectSourceUrl;
    if (typeof url !== "string" || url.length === 0 || newSource.length === 0) return null;
    // 先只读拉当前源码，算真实 旧→新 diff；对象不存在则视为新建。
    try {
      const res = await gen.client.callTool(
        { name: "getObjectSource", arguments: { objectSourceUrl: url } },
        undefined,
      ...
      if (text) {
        try {
          ...
          if (parsed && typeof parsed.source === "string") oldSource = parsed.source;
        } catch (e) {
          oldSource = "";
        }
      }
      if (oldSource.length === 0) throw new Error("no current source");
      ...
      // 对象当前不存在（首次创建）→ 按新建展示完整源码
      const lines = splitDiffLines(newSource);
      return { kind: "create", label: String(label), lineCount: lines.length, source: newSource };
    }
  }
  if (rawToolName === "createObject" || rawToolName === "createTestInclude") {
...
// 生成审批卡 reason：编辑类内联紧凑 diff（真实 +/- 行），新建类显示行数，其余回退摘要。
function writeApprovalReason(rawToolName, args, preview) {
  const tool = publicToolName(rawToolName);
  const head = `写操作确认：${tool} 将向 SAP 写入数据。`;
  if (!preview) {
    return `${head}\n参数：${summarizeWriteArgs(args)}\n\n确认后才会真正执行写入；拒绝则本次不做任何修改。`;
  }
  if (preview.kind === "edit") {
    const shown = preview.hunks.slice(0, MAX_INLINE_REASON_HUNKS);
    let body = `\n对象：${preview.label}  共 ${preview.hunks.length} 处改动（+${preview.add}/-${preview.del}）：`;
    for (const h of shown) {
      body += `\n  @旧 L${h.oldStart} → 新 L${h.newStart}`;
      for (const l of h.lines) {
        const mark = l.t === "old" ? "-" : l.t === "new" ? "+" : " ";
        body += `\n  ${mark} ${l.text}`;
      }
    }
    if (preview.hunks.length > shown.length) {
      body += `\n  … 其余 ${preview.hunks.length - shown.length} 处改动，见下方工具卡「写前预览」，点击序号查看全文`;
    }
    return `${head}${body}\n\n确认后才会真正执行写入；拒绝则本次不做任何修改。`;
  }
  if (preview.kind === "create") {
    return `${head}\n对象：${preview.label}（新建）  源码 ${preview.lineCount} 行。\n完整源码见下方工具卡「写前预览」；确认后才会创建；拒绝则本次不创建。`;
  }
  return `${head}\n对象：${preview.label}\n确认后才会真正执行写入；拒绝则本次不做任何修改。`;
}
...
async function requireWriteApproval(ctx, rawToolName, args, exec, preview) {
  ...
  const outcome = await approval.request({
    ...
    reason: writeApprovalReason(rawToolName, args, preview),
...
export function apply(ctx) {
  ...
  // 写前预览清理：设置命名空间里的预览在 TTL 后清空，避免长期留存源码。
  let previewClearTimer = null;
  function schedulePreviewClear(callId) {
    if (previewClearTimer) {
      clearTimeout(previewClearTimer);
      previewClearTimer = null;
    }
    previewClearTimer = setTimeout(async () => {
      previewClearTimer = null;
      try {
        const cur = read().writePreview;
        if (cur && cur.callId === callId) await scope.update({ writePreview: null });
      } catch (e) {
        /* 清理失败不影响主流程 */
      }
    }, PREVIEW_TTL_MS);
      ...
      definitions.push({
        ...
        execute: async (args, exec) => {
          ...
          if (writeCat !== undefined && read().writeConfirm !== false) {
            // 写前预览：真实 旧→新 diff。完整 hunk 经设置命名空间下发给 Client 工具卡
            // （tool.call.toolview 渲染「序号+点击展开」）；审批卡 reason 内联紧凑 diff。
            let preview = null;
            try {
              preview = await buildWritePreview(gen, t.name, args);
            } catch (e) {
              console.error("[dsh-abap-mcp] build write preview failed:", String(e));
              preview = null;
            }
            const callId = exec && exec.callId ? String(exec.callId) : "";
            if (preview !== null && callId) {
              try {
                await scope.update({ writePreview: { callId, at: Date.now(), tool: t.name, ...preview } });
                schedulePreviewClear(callId);
              } catch (e) {
                console.error("[dsh-abap-mcp] publish write preview failed:", String(e));
              }
            }
            await requireWriteApproval(ctx, t.name, args, exec, preview);
          }
          const res = await gen.client.callTool(
...
// 测试钩子：仅供单测使用（test-*.mjs），不在运行时依赖。
export const __writePreviewTest__ = {
  computeLineDiff,
  writeApprovalReason,
  splitDiffLines,
};
