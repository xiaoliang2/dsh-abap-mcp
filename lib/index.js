...
export function apply(ctx) {
  ...
  const startupTimer = setInterval(() => {
    startupRetries += 1;
    ensureConnected(read(), "startup-retry");
  ...
  ctx.on("settings/updated", (ns) => {
    ...
    configTimer = setTimeout(async () => {
      ...
      if (cfg.testRequest && cfg.testRequest !== lastHandledTestRequest) {
        ...
        if (cfg.enabled) {
          ensureConnected(read(), "after-test").catch((e) =>
  ...
  ctx.on("credentials/reference-updated", (ref) => {
    ...
    configTimer = setTimeout(() => {
      configTimer = null;
      ensureConnected(read(), "password-change");
    }, 250);
  });
  ...
  ctx.tools.register({
    ...
    execute: async () => {
      ...
      return {
        ...
        // 写确认提示：所有写工具执行前都会触发人工审批（DSH 原生审批卡），
        // 未获批准绝不写入。字段用于向模型说明该行为。
        writeApproval: `every write tool (${[...loadWriteCategories().keys()].length} known) requires a user approval before executing`,
      };
    },
  });
